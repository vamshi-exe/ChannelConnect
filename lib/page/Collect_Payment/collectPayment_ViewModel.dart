...
class CollectPaymentViewModel extends BaseViewModel with AppHelper {
  ...
  void collectPayment_invoiceRequest(
    ...
    try {
      final response = await _apiService.collectPayment_invoice(name, email,
          contactNo, amount, referenceNo, description, propertyCode);
      ...
      if (response["ResCreationResult"] != null) {
        ...
        if (response["ResCreationResult"][0]["Desc"] != null) {
          ...
          Navigator.of(context).pop(
              MaterialPageRoute(builder: (context) => CollectPaymentList()));
        }
        DialogHelper.showErrorDialog(context, "Done", result, showTitle: false);
      }
    } catch (e) {
      hideProgressDialog(context);
